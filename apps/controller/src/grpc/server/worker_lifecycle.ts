...
registerService<WorkerLifecycleService, WorkerLifecycleServer>(
  WorkerLifecycleService,
  {
    notifyShutdown: async function (
      ...
      console.log(
        `Received worker lifecycle notification from guild ${guildId}, reason: ${reason}`,
      );
      ...
      if (workers.length === 0) {
        console.warn(
          `No worker found for guild ${guildId} during lifecycle notification`,
        );
      ...
      if (!podName) {
        console.error(
          `Worker found for guild ${guildId} but pod name is missing`,
        );
        callback(null, { acknowledged: false });
        return;
      }
